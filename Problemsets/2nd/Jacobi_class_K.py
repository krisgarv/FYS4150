...
class Eigenvalues():
    ...
    def __init__(self, N, d, a):
        ...
        # Creating the matrix if all diagonals elements are the same:
        if isinstance(d, float or int):
            r[0] = self.d
            r[1] = self.a
            self.M = sl.toeplitz(r, r)
        # Creating the matrix if the diagonal elements are NOT the same:
        else:
            r[1] = self.a
            self.M = sl.toeplitz(r)
...
#---------------------------------------------------------------------
    ...
    def Jacobi(self):
        ...
        epsilon = 1.0e-8
        maxiter = self.N**5  # maximum number of iterations
