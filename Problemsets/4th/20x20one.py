...
def plot_MC_cycles(ordered):
    ...
    for i in range(2):
        ...
        ordered=True
        """
        Energy_T1 = np.zeros_like(steps)
        Energy_T24 = np.zeros_like(steps)
        MagnetizationAbs_T1 = np.zeros_like(steps)
        MagnetizationAbs_T24 = np.zeros_like(steps)
        for i, val in enumerate(steps):
            Energy_T1[i] = energy_avg_T1[int(val)-1]
            Energy_T24[i] = energy_avg_T24[int(val)-1]
            MagnetizationAbs_T1[i] = abs_magnet_avg_T1[int(val)-1]
            MagnetizationAbs_T24[i] = abs_magnet_avg_T24[int(val)-1]
        """
